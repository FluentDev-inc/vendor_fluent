...
- pixelgerrit:     A Git wrapper that fetches/pushes patch from/to fluent Gerrit Review.
- pixelrebase:     Rebase a Gerrit change and push it again.
- fluentremote:      Add git remote for matching fluent repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for fluent Github.
...
- repopick:        Utility to fetch changes from fluent Gerrit.
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/FluentOS-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop org.fluent.device | grep -q "$CUSTOM_BUILD"); then
...
function fluentremote()
{
    ...
    git remote rm fluent 2> /dev/null
    ...
    git remote add fluent https://android.googlesource.com/$PFX$PROJECT
    echo "Remote 'fluent' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/fluent/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop org.fluent.device | grep -q "$CUSTOM_BUILD");
    then
...
function pixelgerrit() {
    ...
    local user=`git config --get review.gerrit.fluent.org.username`
...
function pixelrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "fluent Gerrit Rebase Usage: "
    ...
    git fetch "http://gerrit.fluent.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop org.fluent.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/fluent/build/tools/repopick.py $@
}
